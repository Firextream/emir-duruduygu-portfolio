...
const mockPosts = [
  ...
  },
  {
    ...
    category: "DESIGN",
    readTime: "6 min read",
    image: getAssetPath("/architectural-photography-light-shadow-modern-buil.png"),
  ...
  },
  {
    ...
    readTime: "12 min read",
    image: getAssetPath("/modern-glass-building-reflection.png"),
...
4. Make sure the field is shared with your integration`;
    ...
    const image =
      page.properties?.Image?.files?.[0]?.file?.url || getAssetPath("placeholder.svg");
    const author = 
      page.properties?.Author?.people?.[0]?.name ||
      page.properties?.Author?.rich_text?.[0]?.plain_text ||
      page.properties?.Author?.select?.name ||
