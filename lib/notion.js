  ...
  try {
    ...
    const images = await Promise.all(response.results.map(async (page) => {
      ...
      const getOptimizedUrl = (url, width = 250, quality = 50) => {
        if (!url) return null;
        // Some hosts (signed S3 URLs with X-Amz- params) are not reliably fetched
        // by the public wsrv.nl proxy and return 502 upstream errors. For those
        // we prefer returning the original URL directly and skip proxying.
        const isPresignedS3 = /X-Amz-|s3\.amazonaws\.com|prod-files-secure/.test(url)
        if (isPresignedS3) {
          return url
        }
      ...
      // Determine authoritative source for images (prefer 'onsite' property if present)
      const onsiteProp = page.properties.onsite?.files?.[0]?.external?.url ||
                         page.properties.onsite?.files?.[0]?.file?.url ||
                         page.properties.Onsite?.files?.[0]?.external?.url ||
                         page.properties.Onsite?.files?.[0]?.file?.url ||
                         null;
      const baseOriginal = onsiteProp || originalProp || originalSrc;

      // Stronger blur placeholder (30px width - clearer, less artifacting)
      const blurUrl = baseOriginal ? `https://wsrv.nl/?url=${encodeURIComponent(baseOriginal)}&w=30&blur=15&q=50&output=webp` : null;

      // Probe baseOriginal for dimensions to prevent layout shifts (reuse existing width/height variables)
      if (baseOriginal) {
        try {
          const result = await probe(baseOriginal)
          width = result.width || width
          height = result.height || height
        } catch (e) {
          if (isDev) console.warn("Image probe failed:", baseOriginal, e.message)

          // Fallback: try probing the proxied optimized URL (bigger width)
          try {
            const prox = getOptimizedUrl(baseOriginal, 1600, 85)
            const r2 = await probe(prox)
            // prox returns scaled sizes but preserves aspect ratio
            width = r2.width || width
            height = r2.height || height
      ...
      const thumbSource = baseOriginal;
      const lightboxSource = baseOriginal;
      ...
      return {
        id: page.id,
        src: thumb1x, // Thumbnail - derived from single source
        srcSet: `${thumb1x} 1x, ${thumb2x} 2x`, // Serve 2x for retina
        srcFull: getOptimizedUrl(lightboxSource, 1800, 90), // Lightbox - derived from same source
        srcOriginal: baseOriginal || originalSrc, // Keep original if available
