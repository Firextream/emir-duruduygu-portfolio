...
async function _getGalleryImages() {
  ...
  try {
    ...
    const images = await Promise.all(response.results.map(async (page) => {
      ...
      const originalSrc =
        page.properties.onsite?.files?.[0]?.external?.url ||
        page.properties.onsite?.files?.[0]?.file?.url ||
        page.properties.Image?.files?.[0]?.external?.url ||
        page.properties.Image?.files?.[0]?.file?.url ||
      ...
      if (originalSrc) {
        try {
          ...
          try {
            ...
            if (isDev) console.warn("Proxy probe also failed:", getOptimizedUrl(originalSrc, 1600, 85), e2.message)
          }
        }
      }
      ...
      // Provide SquareSpace-like thumbnails and a srcSet for retina displays
      const thumb1x = getOptimizedUrl(originalSrc, 480, 90)
      const thumb2x = getOptimizedUrl(originalSrc, 960, 90)

      return {
        id: page.id,
        src: thumb1x, // Thumbnail - higher quality for better appearance (~30-70KB)
        srcSet: `${thumb1x} 1x, ${thumb2x} 2x`, // Serve 2x for retina
        srcFull: getOptimizedUrl(originalSrc, 1800, 90), // Lightbox - high quality
