...
async function _getAllPosts() {
  ...
  if (isDirectusConfigured) {
    try {
      const items = await directusReadItems(directusPostsCollection, {
        fields: ["*.*"],
        sort: "-date,-date_published,-published_at",
      });
...
async function _getGalleryImages() {
  ...
  if (isDirectusConfigured) {
    try {
      const items = await directusReadItems(directusGalleryCollection, {
        fields: ["*.*"],
        sort: "-date,-created_at,-sort",
      });
...
async function _getPortfolioItems() {
  ...
  if (isDirectusConfigured) {
    try {
      const items = await directusReadItems(directusPortfolioCollection, {
        fields: ["*.*"],
        sort: "-date,-created_at,-sort",
      });
