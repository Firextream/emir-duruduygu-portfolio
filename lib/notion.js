...
// Cloudinary configuration for global CDN
const CLOUDINARY_CLOUD_NAME = "djl5lgrpr";
const USE_CLOUDINARY = false; // Temporarily disabled - URL encoding issue

// Transform Notion URL to Cloudinary CDN URL for faster global delivery
function getCloudinaryUrl(notionUrl, options = {}) {
  if (!USE_CLOUDINARY || !notionUrl || !notionUrl.startsWith('http')) return notionUrl;
  
  const { width = 800, quality = 'auto', format = 'auto' } = options;
  
  // Cloudinary fetch URL - proxies external images through Cloudinary CDN
  return `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/image/fetch/f_${format},q_${quality},w_${width}/${encodeURIComponent(notionUrl)}`;
}
...
async function _getGalleryImages() {
  ...
  try {
    ...
    const images = response.results.map((page) => {
      ...
      return {
        id: page.id,
        src: getCloudinaryUrl(src, { width: 1200, quality: 'auto', format: 'auto' }),
        srcOriginal: src, // Keep original for download
        alt: name,
        name,
        place,
        category,
