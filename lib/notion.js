...
export async function getGalleryImages() {
  console.log("getGalleryImages called, isGalleryConfigured:", isGalleryConfigured);
  
  // For static export, use mock data in production build
  if (typeof window === 'undefined' && process.env.NODE_ENV === 'production') {
    console.log("Static export mode detected, using mock gallery data");
    return mockGalleryImages;
  }
  
  if (!isGalleryConfigured) {
    console.log("Gallery not configured, using mock data");
    return mockGalleryImages;
  }

  try {
    console.log("Querying Notion gallery database...");
    const response = await notion.databases.query({
      database_id: galleryDatabaseId,
      sorts: [{ property: "Date", direction: "descending" }],
    });

    console.log("Found", response.results.length, "gallery images in Notion");
    
    if (response.results.length === 0) {
      console.log("No images in Notion, using mock data");
      return mockGalleryImages;
    }

    const images = response.results.map((page) => {
      // Debug: log available properties
      console.log("Gallery item properties:", Object.keys(page.properties || {}));
      ...
      const aspectRatio = page.properties.AspectRatio?.select?.name ||
                          page.properties.Aspect?.select?.name ||
                          "4/3";
    ...
    }).filter(image => image.src); // Only return images with valid src

    console.log("Returning", images.length, "valid gallery images");
    return images.length > 0 ? images : mockGalleryImages;
  } catch (error) {
...
export async function getPortfolioItems() {
  console.log("getPortfolioItems called, isPortfolioConfigured:", isPortfolioConfigured);
  
  // For static export, use mock data in production build
  if (typeof window === 'undefined' && process.env.NODE_ENV === 'production') {
    console.log("Static export mode detected, using mock portfolio data");
    return mockPortfolioItems;
  }
  
  if (!isPortfolioConfigured) {
    console.log("Portfolio not configured, using mock data");
    return mockPortfolioItems;
  }

  try {
    console.log("Querying Notion portfolio database...");
    const response = await notion.databases.query({
      database_id: portfolioDatabaseId,
      sorts: [{ property: "Date", direction: "descending" }],
    });

    console.log("Found", response.results.length, "portfolio items in Notion");
    
    if (response.results.length === 0) {
      console.log("No items in Notion, using mock data");
      return mockPortfolioItems;
    }

    const items = response.results.map((page) => {
      console.log("Portfolio item properties:", Object.keys(page.properties || {}));
    ...
    console.log("Returning", items.length, "valid portfolio items");
