...
async function _getGalleryImages() {
  ...
  try {
    ...
    const images = await Promise.all(response.results.map(async (page) => {
      ...
      return {
        id: page.id,
        src: getOptimizedUrl(originalSrc, 250, 70), // Thumbnail - smaller & faster with better quality (~15-25KB)
        srcFull: getOptimizedUrl(originalSrc, 1200, 80), // Lightbox - good quality
