...
async function _getGalleryImages() {
  ...
  try {
    ...
    const images = response.results.map((page) => {
      ...
      // This resizes and compresses images for faster loading
      const getOptimizedUrl = (url, width = 400, quality = 70) => {
        if (!url) return null;
        // wsrv.nl is a free image proxy that handles Notion URLs well
        // fit=cover ensures consistent sizing, we=1 enables WebP
        return `https://wsrv.nl/?url=${encodeURIComponent(url)}&w=${width}&h=${Math.round(width * 0.75)}&fit=cover&q=${quality}&output=webp&il=1`;
      };

      // Tiny blur placeholder (16px)
      const blurUrl = `https://wsrv.nl/?url=${encodeURIComponent(originalSrc)}&w=16&h=12&fit=cover&blur=5&q=30&output=webp`;

      return {
        id: page.id,
        src: getOptimizedUrl(originalSrc, 400, 70), // Thumbnail - small & fast
        srcFull: getOptimizedUrl(originalSrc, 1400, 80), // Lightbox - larger
