...
async function _getGalleryImages() {
  ...
  try {
    ...
    const images = response.results.map((page) => {
      ...
      // Tiny blur placeholder (10px width - super small)
      const blurUrl = `https://wsrv.nl/?url=${encodeURIComponent(originalSrc)}&w=10&blur=5&q=20&output=webp`;

      return {
        id: page.id,
        src: getOptimizedUrl(originalSrc, 350, 65), // Thumbnail - small & fast (~15-30KB)
        srcFull: getOptimizedUrl(originalSrc, 1400, 80), // Lightbox - good quality
