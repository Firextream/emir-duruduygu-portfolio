...
async function _getGalleryImages() {
  ...
  try {
    ...
    const images = await Promise.all(response.results.map(async (page) => {
      ...
      // Tiny blur placeholder (12px width - slightly clearer)
      const blurUrl = `https://wsrv.nl/?url=${encodeURIComponent(originalSrc)}&w=12&blur=5&q=30&output=webp`;

      return {
        id: page.id,
        src: getOptimizedUrl(originalSrc, 350, 80), // Thumbnail - larger & higher quality for better appearance (~20-40KB)
        srcFull: getOptimizedUrl(originalSrc, 1400, 82), // Lightbox - good quality
