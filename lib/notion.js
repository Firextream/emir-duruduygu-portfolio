...
// ============================================
// DIRECTUS CONFIGURATION (Primary CMS when set)
// ============================================
...
const directusPostsCollection = normalizeEnvValue(process.env.DIRECTUS_POSTS_COLLECTION) || "posts";
const directusGalleryCollection = normalizeEnvValue(process.env.DIRECTUS_GALLERY_COLLECTION) || "gallery_images";
const directusPortfolioCollection = normalizeEnvValue(process.env.DIRECTUS_PORTFOLIO_COLLECTION) || "portfolio_items";
const isDirectusConfigured = Boolean(directusUrl);

if (isDev) {
  console.log("Directus Config Debug:", {
    enabled: isDirectusConfigured,
    hasToken: Boolean(directusToken),
    postsCollection: directusPostsCollection,
    galleryCollection: directusGalleryCollection,
    portfolioCollection: directusPortfolioCollection,
  });
}
...
function toReadTime(content, fallback = "5 min read") {
...
function toBool(value) {
  ...
  if (typeof value === "string") {
    return ["1", "true", "yes", "on", "selected"].includes(value.toLowerCase());
  }
  return false;
}
...
function directusAssetId(fileValue) {
  ...
  if (typeof fileValue === "object") {
    const candidate =
      fileValue.id ||
      fileValue.directus_files_id?.id ||
      fileValue.directus_files_id ||
      fileValue.file?.id ||
      fileValue.file;
...
function directusProxyUrl(url) {
  if (!url) return null;
  return `/api/image-proxy?url=${encodeURIComponent(url)}`;
}

function directusAssetVariantUrl(fileValue, query) {
  const id = directusAssetId(fileValue);
  if (!id) {
    return directusAssetUrl(fileValue);
  }

  const base = joinUrl(directusUrl, `/assets/${id}`);
  const raw = query ? `${base}?${query}` : base;
  return directusProxyUrl(raw);
}

function directusAssetUrl(fileValue) {
  if (!fileValue) return null;

  if (typeof fileValue === "string") {
    if (/^https?:\/\//i.test(fileValue)) return fileValue;
    return joinUrl(directusUrl, `/assets/${fileValue}`);
  }
  ...
  if (typeof fileValue === "object") {
    ...
    if (typeof candidate === "string") {
      ...
      return joinUrl(directusUrl, `/assets/${candidate}`);
    }
  }
...
async function directusReadItems(collection, { fields = ["*"], filter, sort, limit } = {}) {
  if (!isDirectusConfigured) return [];

  const url = new URL(joinUrl(directusUrl, `/items/${collection}`));
...
function isDirectusSortOrFieldError(error) {
  const message = String(error?.message || "").toLowerCase();
  return (
    message.includes("forbidden") ||
    message.includes("permission to access fields") ||
    message.includes("do not exist")
  );
}

async function directusReadItemsWithSortFallback(
  collection,
  { fields = ["*"], filter, limit } = {},
  sortCandidates = []
) {
  if (!Array.isArray(sortCandidates) || sortCandidates.length === 0) {
    return directusReadItems(collection, { fields, filter, limit });
  }
  ...
  for (const sort of sortCandidates) {
    try {
      return await directusReadItems(collection, { fields, filter, limit, sort });
    } catch (error) {
      lastError = error;
      if (!isDirectusSortOrFieldError(error)) {
        throw error;
      }
      if (isDev) {
        console.warn(`[Directus] sort fallback triggered for ${collection}:`, sort);
      }
    }
  }

  if (isDev) {
    console.warn(`[Directus] falling back to unsorted query for ${collection}`);
  }

  try {
    return await directusReadItems(collection, { fields, filter, limit });
  } catch (error) {
    throw lastError || error;
  }
}

function mapDirectusPost(item) {
  const title = item.title || item.name || "Untitled";
  const content = item.content || item.body || "";
  const excerpt = item.excerpt || (content ? `${content.slice(0, 200)}...` : "");
  const slug = toSlug(item.slug || title);
  const image = directusAssetUrl(item.cover_image || item.image || item.thumbnail);
  ...
  return {
    ...
    slug,
    excerpt,
    date: item.date || item.date_published || item.published_at || null,
    category: item.category || "Uncategorized",
    readTime: item.read_time || toReadTime(content),
    image,
    featured: toBool(item.featured),
    content,
    author: item.author_name || item.author || "Duruduygu",
    authorTitle: item.author_title || "Blogger",
    authorImage: directusAssetUrl(item.author_image),
  };
}

function mapDirectusGalleryItem(item) {
  const name = item.name || item.title || "Untitled";
  const sourceFile = item.image || item.file || item.photo || item.cover;
  const originalSrc = directusAssetVariantUrl(sourceFile);
  const status = String(item.status || "published").toLowerCase();

  if (!originalSrc || ["draft", "archived"].includes(status)) {
    return null;
  }
  ...
  const exif = {
    ...
    shutterSpeed: item.shutter_speed || null,
    iso: item.iso ? String(item.iso) : null,
    focalLength: item.focal_length || null,
  };
  const hasExif = Object.values(exif).some((v) => v !== null);

  return {
    id: item.id,
    // Use transformed variants for faster grid/lightbox while preserving original.
    src: directusAssetVariantUrl(sourceFile, "width=1200&quality=75&format=webp"),
    srcSet: [
      `${directusAssetVariantUrl(sourceFile, "width=900&quality=72&format=webp")} 900w`,
      `${directusAssetVariantUrl(sourceFile, "width=1200&quality=75&format=webp")} 1200w`,
      `${directusAssetVariantUrl(sourceFile, "width=1800&quality=78&format=webp")} 1800w`,
    ].join(", "),
    srcFull: directusAssetVariantUrl(sourceFile, "width=2200&quality=82&format=jpg"),
    srcOriginal: directusAssetVariantUrl(sourceFile),
    blurDataUrl: null,
    width: item.width || null,
    height: item.height || null,
    aspectRatio: normalizeAspectRatio(item.aspect_ratio || item.aspectRatio || null),
    alt: item.alt || name,
    name,
    place: item.place || item.location || "Unknown",
    category: item.category || "Photography",
    selected: toBool(item.selected),
    featured: toBool(item.featured),
    exif: hasExif ? exif : null,
  };
}

function mapDirectusPortfolioItem(item) {
  const name = item.name || item.title || "Untitled";
  const src = directusAssetUrl(item.image || item.file || item.cover);
  const status = String(item.status || "published").toLowerCase();

  if (!src || ["draft", "archived"].includes(status)) {
    return null;
  }
  ...
  return {
    ...
    alt: item.alt || name,
    name,
    date: item.date || item.year || new Date().getFullYear().toString(),
    place: item.place || item.location || "Unknown",
    category: item.category || "Photography",
...
// ============================================
// LAST GOOD CACHE (avoid falling back to mock on transient errors)
// ============================================
const lastGood = {
  posts: null,
  gallery: null,
...
// ============================================
// NOTION RETRY / LOGGING
// ============================================
const NOTION_RETRY_MAX = Number.parseInt(process.env.NOTION_RETRY_MAX || "3", 10);
const NOTION_RETRY_BASE_MS = Number.parseInt(process.env.NOTION_RETRY_BASE_MS || "500", 10);

function getNotionErrorInfo(error) {
  return {
...
function isRetryableNotionError(error) {
  ...
  if (["rate_limited", "internal_server_error", "service_unavailable", "gateway_timeout"].includes(code)) return true;
  return false;
}
...
async function withNotionRetry(fn, context) {
  let attempt = 0;
  ...
  while (true) {
    try {
      ...
      const info = getNotionErrorInfo(error);
      const shouldRetry = isRetryableNotionError(error);
      const maxAttempts = NOTION_RETRY_MAX + 1;
      const attemptLabel = `${attempt + 1}/${maxAttempts}`;

      console.warn(`[Notion] ${context} failed (attempt ${attemptLabel})`, info);
      ...
      const backoff = NOTION_RETRY_BASE_MS * Math.pow(2, attempt - 1);
...
async function queryNotionDatabase(params, context) {
  return withNotionRetry(
    () => notion.databases.query(params),
    context || `databases.query (${params?.database_id || "unknown"})`
  );
}
...
function getUrlFromProperty(prop) {
  ...
  if (prop.type === "rich_text" && Array.isArray(prop.rich_text)) {
    const text = prop.rich_text.map((segment) => segment?.plain_text || "").join("").trim();
    if (/^https?:\/\//i.test(text)) return text;
  }
  return null;
}

function getUrlFromProperties(properties, keys) {
  if (!properties || !Array.isArray(keys)) return null;
  for (const key of keys) {
    const url = getUrlFromProperty(properties[key]);
    if (url) return url;
  }
  return null;
}

function findFirstUrl(properties) {
...
function getTextFromProperty(prop) {
  if (!prop) return null;
  if (prop.type === "rich_text" && Array.isArray(prop.rich_text)) {
    return prop.rich_text.map((segment) => segment?.plain_text || "").join("").trim() || null;
  }
  if (prop.type === "title" && Array.isArray(prop.title)) {
    return prop.title.map((segment) => segment?.plain_text || "").join("").trim() || null;
  }
  if (prop.type === "select") {
    return prop.select?.name || null;
  }
  if (prop.type === "multi_select") {
    const names = prop.multi_select?.map((opt) => opt?.name).filter(Boolean) || [];
  ...
  if (prop.type === "formula") {
    return prop.formula?.string || (prop.formula?.number !== null && prop.formula?.number !== undefined ? String(prop.formula.number) : null);
  }
  return null;
}
...
async function _getPostBySlug(slug) {
  ...
  if (isDirectusConfigured) {
    try {
      const items = await directusReadItems(directusPostsCollection, {
        fields: ["*"],
        filter: {
          _or: [
            { slug: { _eq: slug } },
            { slug: { _icontains: slug } },
            { title: { _icontains: slug.replace(/-/g, " ") } },
          ],
        },
        limit: 10,
      });
      ...
      const cachedPost = Array.isArray(lastGood.posts)
        ? lastGood.posts.find((item) => item?.slug === slug)
        : null;
      return cachedPost || null;
    } catch (error) {
      console.error("Error fetching Directus post by slug:", error);
      const cachedPost = Array.isArray(lastGood.posts)
        ? lastGood.posts.find((item) => item?.slug === slug)
        : null;
      return cachedPost || null;
  ...
  try {
    ...
    const allPostsResponse = await queryNotionDatabase(
      {
    ...
    if (matchingPost) {
      ...
      const cachedPost = Array.isArray(lastGood.posts)
        ? lastGood.posts.find((post) => post?.slug === slug)
        : null;
      return cachedPost || null;
    }
  } catch (error) {
...
async function _getAllPosts() {
  ...
  if (isDirectusConfigured) {
    try {
      const items = await directusReadItemsWithSortFallback(
        directusPostsCollection,
        {
          fields: ["*"],
        },
        ["-date"]
      ...
      const posts = items.map(mapDirectusPost).filter(Boolean);
      if (posts.length > 0) {
        lastGood.posts = posts;
        return posts;
      }
      return Array.isArray(lastGood.posts) ? lastGood.posts : [];
    } catch (error) {
      console.error("Error fetching Directus posts:", error);
      return Array.isArray(lastGood.posts) ? lastGood.posts : mockPosts;
    }
  }
...
4. Make sure the field is shared with your integration`;
    ...
    const image =
      getUrlFromProperties(page.properties, [
        "Image",
        "image",
        "Cover",
        "cover",
...
async function _getGalleryImages() {
  ...
  if (isDev) console.log("Gallery Config:", { isGalleryConfigured, galleryDatabaseId, hasToken: !!notionToken });

  if (isDirectusConfigured) {
    try {
      const items = await directusReadItemsWithSortFallback(
        directusGalleryCollection,
        {
          fields: ["*"],
        },
        ["-date"]
      ...
      const images = items.map(mapDirectusGalleryItem).filter(Boolean);
      if (images.length > 0) {
        lastGood.gallery = images;
        return images;
      }
      return Array.isArray(lastGood.gallery) ? lastGood.gallery : [];
    } catch (error) {
      console.error("Error fetching Directus gallery images:", error);
      return Array.isArray(lastGood.gallery) ? lastGood.gallery : mockGalleryImages;
    }
  }
  
  if (!isGalleryConfigured) {
    if (isDev) console.warn("Notion gallery DB not configured - returning mock images");
  ...
  try {
    ...
    const images = await Promise.all(response.results.map(async (page) => {
      ...
      // Try many possible property names that projects sometimes use
      let originalSrc =
        getUrlFromProperties(page.properties, [
          "onsite",
          "Image",
      ...
      if (!originalSrc) {
        originalSrc = findFirstUrl(page.properties);
      }
      
      if (isDev) console.log(`Gallery image ${name}: onsite=${!!page.properties.onsite?.files?.[0]}, originalSrc=${originalSrc ? 'found' : 'null'}`);
      if (isDev && !originalSrc) console.log('Gallery page properties keys:', Object.keys(page.properties || {}));
      ...
      const aspectRatioText =
        getTextFromProperty(page.properties?.AspectRatio) ||
        getTextFromProperty(page.properties?.aspectRatio) ||
        getTextFromProperty(page.properties?.Ratio) ||
        getTextFromProperty(page.properties?.ratio) ||
        null;

      const aspectRatio = normalizeAspectRatio(aspectRatioText);
      ...
      const selected =
        selectedProp?.type === "checkbox"
          ? selectedProp.checkbox
          : selectedProp?.type === "select"
            ? ["selected", "yes", "true", "1"].includes(
                (selectedProp.select?.name || "").toLowerCase()
              )
            : selectedProp?.type === "multi_select"
              ? selectedProp.multi_select?.some((opt) =>
                  ["selected", "yes", "true", "1"].includes((opt.name || "").toLowerCase())
                )
              : false;
      ...
      // Proxy images through our API to avoid referrer/CORS and expired signed URL issues
      const getProxyUrl = (url) => {
        if (!url) return null;
        return `/api/image-proxy?url=${encodeURIComponent(url)}`
      };

      const thumb1x = getProxyUrl(originalSrc)

      return {
        id: page.id,
        src: thumb1x, // Proxy URL for display
        srcSet: undefined,
        srcFull: thumb1x, // Use same proxy URL for lightbox
        srcOriginal: originalSrc, // Keep original for download
        blurDataUrl: null,
        width,
        height,
        aspectRatio,
    ...
    if (isDev && imagesFiltered.length === 0) {
      console.warn('Gallery mapping produced 0 images after filtering src; total results:', response.results.length);
      console.warn('First page properties keys:', Object.keys(response.results[0]?.properties || {}));
      console.warn('First page properties sample:', JSON.stringify(response.results[0]?.properties || {}, null, 2).substring(0, 1000));
    }

    if (imagesFiltered.length > 0) {
      lastGood.gallery = imagesFiltered;
...
export const getGalleryImages = unstable_cache(
  async () => {
    if (isDev) console.log("getGalleryImages cache wrapper called");
    return _getGalleryImages();
  },
  ['gallery-images'],
  { revalidate: 300, tags: ['gallery'] }
);

// Uncached gallery fetch for routes/components that must refresh signed URLs immediately.
export async function getGalleryImagesFresh() {
  return _getGalleryImages();
}
...
async function _getPortfolioItems() {
  ...
  if (isDirectusConfigured) {
    try {
      ...
      const mapped = items.map(mapDirectusPortfolioItem).filter(Boolean);
      if (mapped.length > 0) {
        lastGood.portfolio = mapped;
        return mapped;
      }
      return Array.isArray(lastGood.portfolio) ? lastGood.portfolio : [];
    } catch (error) {
      console.error("Error fetching Directus portfolio items:", error);
      return Array.isArray(lastGood.portfolio) ? lastGood.portfolio : mockPortfolioItems;
    }
  }
