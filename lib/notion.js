  ...
  try {
    ...
    const images = await Promise.all(response.results.map(async (page) => {
      ...
      const getFileUrl = (file) =>
        file?.external?.url || file?.file?.url || null;

      const findFirstFileUrl = (properties) => {
        if (!properties) return null;
        for (const key of Object.keys(properties)) {
          const prop = properties[key];
          if (prop?.type === "files" && Array.isArray(prop.files) && prop.files.length > 0) {
            const url = getFileUrl(prop.files[0]);
            if (url) return url;
          }
        }
        return null;
      ...
      let originalSrc =
        getFileUrl(page.properties.onsite?.files?.[0]) ||
        getFileUrl(page.properties.Image?.files?.[0]) ||
        getFileUrl(page.properties.Images?.files?.[0]) ||
        getFileUrl(page.properties.image?.files?.[0]) ||
        getFileUrl(page.properties.Photo?.files?.[0]) ||
        getFileUrl(page.properties.Cover?.files?.[0]) ||
        null;

      if (!originalSrc) {
        originalSrc = findFirstFileUrl(page.properties);
      }
