  ...
  try {
    ...
    const images = response.results.map((page) => {
      ...
      const getOptimizedUrl = (url, width = 400, quality = 70) => {
        if (!url) return null;
        // wsrv.nl - only set width, let height be auto to preserve aspect ratio
        return `https://wsrv.nl/?url=${encodeURIComponent(url)}&w=${width}&q=${quality}&output=webp`;
      };

      // Tiny blur placeholder (20px width, auto height)
      const blurUrl = `https://wsrv.nl/?url=${encodeURIComponent(originalSrc)}&w=20&blur=5&q=30&output=webp`;

      return {
        id: page.id,
        src: getOptimizedUrl(originalSrc, 500, 75), // Thumbnail - good quality
        srcFull: getOptimizedUrl(originalSrc, 1600, 85), // Lightbox - high quality
