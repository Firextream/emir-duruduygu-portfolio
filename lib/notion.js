...
async function _getGalleryImages() {
  ...
  try {
    ...
    const images = await Promise.all(response.results.map(async (page) => {
      ...
      // Better blur placeholder (20px width - clearer)
      const blurUrl = `https://wsrv.nl/?url=${encodeURIComponent(originalSrc)}&w=20&blur=10&q=40&output=webp`;

      // Provide high-quality thumbnails and a srcSet for retina displays
      const thumb1x = getOptimizedUrl(originalSrc, 420, 85)
      const thumb2x = getOptimizedUrl(originalSrc, 840, 85)

      return {
        id: page.id,
        src: thumb1x, // Thumbnail - higher quality for better appearance (~25-60KB)
